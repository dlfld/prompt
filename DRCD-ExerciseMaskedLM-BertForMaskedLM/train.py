...
if __name__ == "__main__":
    ...
    # PreprocessData
    train_data_feature = convert_data_to_feature('/home/dlf/prompt/DRCD-ExerciseMaskedLM-BertForMaskedLM/DRCD_training.json')
    test_data_feature = convert_data_to_feature('/home/dlf/prompt/DRCD-ExerciseMaskedLM-BertForMaskedLM/DRCD_test.json')
    train_dataset = makeDataset(input_ids = train_data_feature['input_ids'], loss_ids = train_data_feature['labels'])
    test_dataset = makeDataset(input_ids = test_data_feature['input_ids'], loss_ids = test_data_feature['labels'])
    ...
    for epoch in range(training_epoch):
        ...
        for batch_index, batch_dict in enumerate(train_dataloader):
            ...
            print(batch_dict)
            batch_dict = torch.cat(batch_dict[0],batch_dict[1])
            outputs = model(
                batch_dict
            )
