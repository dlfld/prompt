...
def convert_data_to_feature(filepath):
    ...
    tokenizer = BertTokenizer(vocab_file='/home/dlf/prompt/DRCD-ExerciseMaskedLM-BertForMaskedLM/bert-base-chinese-vocab.txt')

    context_tokens = []
    context_loss_tokens = []
    max_seq_len = 0
    for context in datasets:
    # BertForMaskedLM的訓練需要特殊符號('[MASK]')以及被mask掉的詞的id
        conversion_context(context, tokenizer, max_seq_len, context_tokens, context_loss_tokens)
...
def conversion_context(context, tokenizer, max_seq_len, context_tokens, context_loss_tokens):
    ...
    while len(Mask_id_list) == 0:
        ...
        new_word_piece_list = word_piece_list
        Mask_id_list.append(tokenizer.convert_tokens_to_ids(context.split("→")[1]))
