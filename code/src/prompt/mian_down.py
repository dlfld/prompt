...
def train_model(train_data, test_data, model, tokenizer, train_loc, data_size, fold):
    ...
    import csv
    dir = Config.loss_dir
    with open(f'{dir}/{pre_train_model_name}_{data_size}_{fold}_train.csv', 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerows(loss_list)
    with open(f'{dir}/{pre_train_model_name}_{data_size}_{fold}_test.csv', 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
...
def train(model_checkpoint, few_shot_start, data_index):
    ...
    for few_shot_idx in range(few_shot_start, len(Config.few_shot)):
        ...
        for index, standard_data_train in enumerate(train_data_all):
            ...
            # 加载继续预训练的模型
            logddd.log("加载继续预训练模型:"+Config.continue_plm_file)
            model = torch.load(Config.continue_plm_file,map_location=Config.device)
            new_parameter =  nn.Parameter(model.transition_params[:Config.class_nums, :Config.class_nums].clone()).to(Config.device)
            #del model_new
            model.transition_params = nn.Parameter(new_parameter.to(Config.device))
